...
from metrics import gather_data, compute_metrics, classify_acc
...
from metrics import INCEPTION_V3_FID, INCEPTION_V3, get_activations, calculate_frechet_distance, calculate_activation_statistics
...
def train_one_epoch(
    ...
    for batch in logger.log_every(data_loader, print_freq, header):
        ...
        with amp_autocast():
            if add_mask:
                if with_regression:
                    # ab_pred, q_pred, q_actual, out_feature = model.forward(img_l, img_ab, img_mask)       # out_feature: [B, 2, 256, 256]:[-1, 1]
                    ab_pred, q_pred, q_actual, out_feature, correct_ab = model.forward(img_l, img_ab, img_mask)
                else:
                    # ab_pred, q_pred, q_actual, out_feature = model.forward(img_l, img_ab, img_mask)       # out_feature: [B, 2, 256, 256]:[-1, 1]
                    ab_pred, q_pred, q_actual, out_feature = model.forward(img_l, img_ab, img_mask)
            ...
            if add_l1_loss:
                if l1_conv:
                    ...
                    # print("norm_ab.shape:", norm_ab.shape)
                    loss_l1 = loss_fn_l1(norm_ab, out_feature)
                    # loss_l1 = loss_fn_l1(img_ab/110, ab_pred/110)

                    # print("img_ab", img_ab)
                    # print("ab_pred", ab_pred)
                elif l1_linear:
                    norm_ab = img_ab / 110.
                    loss_l1 = loss_fn_l1(norm_ab, out_feature)
            ...
            if with_regression:
                dis_ang = regression.ang_dis_count(img_ab, ab_pred)  # 真实ab值（前面）减预测ab值（后面）
                dis_ang_loss_l1 = loss_fn_l1(correct_ab, dis_ang)
                loss += dis_ang_loss_l1
            # print("dis_ang_loss_l1:", dis_ang_loss_l1)
            # print("loss:", loss)

            # if add_edge:
            #     img_pred = torch.cat((img_l, ab_pred), dim=1)
            #     img_gt = torch.cat((img_l, img_ab), dim=1)
            #     pred_edge = F.laplacian(img_pred, 3)
            #     gt_edge = F.laplacian(img_gt, 3)
            #     loss += loss_fn_l1(pred_edge, gt_edge)
    ...
    loss_l1 = train_l1_total / len(data_loader)
    learning_rate = optimizer.param_groups[0]["lr"]
    return logger, loss, loss_l1, learning_rate
...
def evaluate(
    ...
    with torch.no_grad():
        for batch in logger.log_every(data_loader, print_freq, header):
            ...
            img_ab = img_ab.to(ptu.device)
            if add_mask: img_mask = img_mask.to(ptu.device)
            # print("eval with_regression", with_regression)

            with amp_autocast():
                if add_mask:
                    if with_regression:
                        ab_pred, q_pred, q_actual, out_feature, correct_ab = model_without_ddp.inference(img_l, img_ab, img_mask)
                        d = correct_ab[:, 0, ]
                        theta = correct_ab[:, 1, ]
                        a = d * torch.cos(theta)
                        b = d * torch.sin(theta)
                        ...
                        correct_ab = torch.cat((a, b), dim=1)
                        ab_pred = ab_pred + correct_ab
                        # print("correct_ab", correct_ab)
                    else:
                        ab_pred, q_pred, q_actual, out_feature = model_without_ddp.inference(img_l, img_ab,
                                                                                                         img_mask)
